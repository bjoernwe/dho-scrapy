...
class MessageDB:
    ...
    def group_by_author(
        self, keep_authors: Optional[Set[str]] = None, min_num_messages: int = 1
    ) -> Dict[str, "MessageDB"]:
        return self._group_messages(
            key=lambda m: m.author,
            keep_keys=keep_authors,
            min_group_size=min_num_messages,
        )

    def group_by_category(
        self, keep_categories: Optional[Set] = None, min_num_messages: int = 1
    ) -> Dict[str, "MessageDB"]:
        return self._group_messages(
    ...
    def group_by_thread(self, min_num_messages: int = 1) -> Dict[int, "MessageDB"]:
        thread_msgs = self._group_messages(
    ...
    def filter_thread_responses(self, keep_op: bool) -> "MessageDB":
        filtered_msgs = [
            msg for msg in self.get_all_messages() if msg.is_first_in_thread
        ]
        if keep_op:
    ...
    def filter_authors(
        ...
        author_msgs = self.group_by_author(
            keep_authors=authors, min_num_messages=min_num_messages
        )
        filtered_msgs = [
            msg
            for author, msgs in author_msgs.items()
    ...
    def filter_categories(
        ...
        filtered_msgs = [
            msg
            for category, msgs in category_msgs.items()
            for msg in msgs.get_all_messages()
        ]
        return MessageDB(msgs=filtered_msgs)
