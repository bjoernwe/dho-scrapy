...
@pytest.fixture(scope='session')
def jsonl_tmp_path(session_tmp_path: Path) -> Path:
    return session_tmp_path.joinpath('items.jsonl')
...
@pytest.fixture(scope='session')
def crawled_messages(jsonl_tmp_path: Path, test_dho_category: DhOCategory) -> List[DhOMessage]:

    settings = get_project_settings()
    settings['FEEDS'][str(jsonl_tmp_path)] = {'format': 'jsonlines'}
    ...
    return _messages_from_file(jsonl_tmp_path)
